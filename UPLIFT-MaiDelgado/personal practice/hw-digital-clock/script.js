...
function showTime() {
  ...
  let sec = time.getSeconds();
  let am_pm = "AM";
  let timeZone = time.toString().split(" ").slice(5, time.length).join(" ");
  
  if (timeFormat.innerHTML === "Standard Time") {
    if (hour > 12) {
      hour -= 12;
      am_pm = "PM";
    } else if (hour == 0) {
...
timeFormat.addEventListener("click", function(){
  if (timeFormat.innerHTML === "Standard Time"){
    ...
    // clock.innerHTML = `${toMilitary(hour)}:${min}:${sec}${am_pm}`;
  } else {
    timeFormat.innerHTML = "Standard Time";
    // clock.innerHTML =`${toTwelveHour(hour)}:${min}:${sec}${am_pm}`;
  }
})
